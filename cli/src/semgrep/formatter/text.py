...
class TextFormatter(BaseFormatter):
    ...
    def _dataflow_trace_to_lines(
        dataflow_trace: Optional[out.DataflowTrace],
        color_output: bool,
        per_finding_max_lines_limit: Optional[int],
        per_line_max_chars_limit: Optional[int],
    ) -> Iterator[str]:
        if dataflow_trace:
            ...
            if source and len(source) > 0:
                ...
                # TODO sort instead of relying on locations appearing in order?
                first_loc = source[0]
                last_loc = source[-1]
                path = Path(first_loc.path)
                lines = get_lines(path, first_loc.start.line, last_loc.end.line)
